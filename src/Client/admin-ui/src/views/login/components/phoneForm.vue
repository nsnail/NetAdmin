<template>
    <el-form ref="loginForm" :model="form" :rules="rules" label-width="0" size="large" @keyup.enter="login">
        <el-form-item prop="destMobile">
            <el-input v-model="form.destMobile" :placeholder="$t('login.mobilePlaceholder')" clearable
                      maxlength="11"
        ...
        <el-form-item prop="code" style="margin-bottom: 35px;">
            <div class="login-msg-yzm">
                <el-input v-model="form.code" :placeholder="$t('login.smsPlaceholder')" clearable
                          maxlength="4"
                          oninput="value=value.replace(/[^\d]/g,'')"
...
export default {
    ...
    methods: {
        async captchaSuccess(obj) {
            ...
            try {
                await this.$API.sys_sms.sendSmsCode.post({
                    ...
                    type: 'login', verifyCaptchaReq: obj
                })
        ...
        async getYzm() {
            var validate = await this.$refs.loginForm.validateField("destMobile").catch(() => {
            })
            if (!validate) {
                return false
            }
