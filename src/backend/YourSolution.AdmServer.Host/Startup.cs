...
namespace YourSolution.AdmServer.Host
{
    ...
    public sealed class Startup : NetAdmin.Host.Startup, ICommand<CommandLineArgs>
    {
        ...
        public async Task<int> ExecuteAsync(
            CommandContext context
            , CommandLineArgs settings
        ) {
            Args = settings;
            var webOpt = new WebApplicationOptions
            {
                EnvironmentName = Environment.GetEnvironmentVariable("TEST_ENVIRONMENT").NullOrEmpty(null), Args = context.Remaining.Raw.ToArray()
            };
