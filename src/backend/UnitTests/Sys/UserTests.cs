    ...
    : WebApiTestBase<Startup>(factory, testOutputHelper), IUserModule
{
    ...
    public async Task<QueryUserRsp> CreateAsync(CreateUserReq req) {
        var rsp = await PostJsonAsync(
            ...
            , new CreateUserReq { UserName = "test", PasswordText = "1234qwer", RoleIds = [371729946431493], DeptId = 372119301627909 }
        );
        Assert.True(rsp.IsSuccessStatusCode);
        return null;
    }
    ...
    public async Task<QueryUserRsp> EditAsync(EditUserReq req) {
        var rsp = await PostJsonAsync(
            typeof(UserController), new EditUserReq { UserName = "test", RoleIds = [371729946431493], DeptId = 372119301627909 }
        );
        Assert.True(rsp.IsSuccessStatusCode);
        return null;
    }
