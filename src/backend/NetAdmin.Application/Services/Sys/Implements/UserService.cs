...
using NetAdmin.Application.Caches.Sys;
...
public class UserService : RepositoryService<TbSysUser, IUserService>, IUserService, IDynamicApiController
{
    ...
    /// </summary>
    public UserService(Repository<TbSysUser> rpo, IUserCache userCache) //
        : base(rpo)
    {
    ...
    {
        return await _userCache.UserInfo(async () => {
            var dbUser = await Rpo.Where(a => a.Token == User.Token && (a.BitSet & (long)Enums.BitSets.Enabled) == 1)
                                  .Include(a => a.Dept)
                                  .IncludeMany( //
                                      a => a.Roles, then =>
                                          then.Where(a => (a.BitSet & (long)Enums.BitSets.Enabled) == 1)
                                              .IncludeMany( //
                                                  a => a.Menus
                                                , menuThen =>
                                                      menuThen.Where(
                                                          a => (a.BitSet & (long)Enums.BitSets.Enabled) == 1))
                                              .IncludeMany( //
                                                  a => a.Depts
                                                , deptThen =>
                                                      deptThen.Where(
