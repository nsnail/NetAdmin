...
public class DeptApi : RepositoryApi<TbSysDept, IDeptApi>, IDeptApi
{
    ...
    /// </summary>
    public DeptApi(Repository<TbSysDept> repo) //
        : base(repo) { }
    ...
    {
        if (req.ParentId != 0 && !await Repo.Select.AnyAsync(a => a.Id == req.ParentId)) {
    ...
    {
        var ret = await Repo.Select.WhereDynamicFilter(req.DynamicFilter)
