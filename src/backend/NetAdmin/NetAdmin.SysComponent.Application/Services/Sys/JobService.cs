    ...
    : RepositoryService<Sys_Job, long, IJobService>(rpo), IJobService
{
    ...
    public async Task<IOrderedEnumerable<KeyValuePair<IImmutableDictionary<string, string>, int>>> CountByAsync(QueryReq<QueryJobReq> req) {
        ...
        return ret
            .Select(x => new KeyValuePair<IImmutableDictionary<string, string>, int>(
                    req.RequiredFields.ToImmutableDictionary(y => y, y => typeof(Sys_Job).GetProperty(y)!.GetValue(x.Key)?.ToString()), x.Value
                )
            )
    ...
    public async Task<QueryJobRsp> CreateAsync(CreateJobReq req) {
        ...
        var ret = await Rpo
            .InsertAsync(
                req with { NextExecTime = nextExecTime, NextTimeId = nextExecTime?.TimeUnixUtc(), RequestHeader = req.RequestHeaders?.Json() }
            )
            .ConfigureAwait(false);
    ...
    public async Task<QueryJobRsp> EditAsync(EditJobReq req) {
        req.ThrowIfInvalid();
        var update = Rpo
            .UpdateDiy.Set(a => a.ExecutionCron == req.ExecutionCron)
    ...
    public async Task ExecuteAsync(QueryJobReq req) {
        ...
        var df = new DynamicFilterInfo
        {
            Filters =
            [
                new DynamicFilterInfo { Field = nameof(QueryJobReq.Enabled), Operator = DynamicFilterOperators.Eq, Value = true }
                , new DynamicFilterInfo { Field = nameof(QueryJobReq.Status), Operator = DynamicFilterOperators.Eq, Value = JobStatues.Idle }
            ]
        };
        var job = await QueryInternal(new QueryReq<QueryJobReq> { Count = 1, Filter = req, DynamicFilter = df, Order = Orders.None })
                      .ToOneAsync()
                      .ConfigureAwait(false)
                  ?? throw new NetAdminInvalidOperationException(Ln.未获取到待执行任务);
    ...
    public async Task<QueryJobRsp> GetNextJobAsync() {
        var df = new DynamicFilterInfo
        {
            ...
            [
                ...
                {
                    Field = nameof(QueryJobReq.NextExecTime), Value = DateTime.Now, Operator = DynamicFilterOperators.LessThan
                }
                , new DynamicFilterInfo { Field = nameof(QueryJobReq.Status), Value = JobStatues.Idle, Operator = DynamicFilterOperators.Eq }
                , new DynamicFilterInfo { Field = nameof(QueryJobReq.Enabled), Value = true, Operator = DynamicFilterOperators.Eq }
            ]
        };
    ...
    private ISelect<Sys_Job> QueryInternal(
        QueryReq<QueryJobReq> req
        , bool includeUser
    ) {
        var ret = Rpo.Select;
        if (includeUser) {
