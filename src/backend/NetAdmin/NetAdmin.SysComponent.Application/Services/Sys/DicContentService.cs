    ...
    : RepositoryService<Sys_DicContent, long, IDicContentService>(rpo), IDicContentService
{
    ...
    public async Task<IOrderedEnumerable<KeyValuePair<IImmutableDictionary<string, string>, int>>> CountByAsync(QueryReq<QueryDicContentReq> req) {
        ...
        return ret
            .Select(x => new KeyValuePair<IImmutableDictionary<string, string>, int>(
                    req.RequiredFields.ToImmutableDictionary(y => y, y => typeof(Sys_DicContent).GetProperty(y)!.GetValue(x.Key)?.ToString()), x.Value
                )
            )
    ...
    private ISelect<Sys_DicContent> QueryInternal(QueryReq<QueryDicContentReq> req) {
        var ret = Rpo
            ...
            .WhereIf(
                req.Keywords?.Length > 0, a => a.Key.Contains(req.Keywords) || a.Value.Contains(req.Keywords) || a.Summary.Contains(req.Keywords)
            );
