    ...
    : RepositoryService<Sys_Role, long, IRoleService>(rpo), IRoleService
{
    ...
    public async Task<IOrderedEnumerable<KeyValuePair<IImmutableDictionary<string, string>, int>>> CountByAsync(QueryReq<QueryRoleReq> req) {
        ...
        return ret
            .Select(x => new KeyValuePair<IImmutableDictionary<string, string>, int>(
                    req.RequiredFields.ToImmutableDictionary(y => y, y => typeof(Sys_Role).GetProperty(y)!.GetValue(x.Key)?.ToString()), x.Value
                )
            )
    ...
    private ISelect<Sys_Role> QueryInternal(QueryReq<QueryRoleReq> req) {
        ...
        var ret = Rpo
            .Select.IncludeMany(a => Enumerable.Select(a.Depts, b => new Sys_Dept { Id = b.Id }))
