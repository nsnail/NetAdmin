...
public sealed class ConfigService(BasicRepository<Sys_Config, long> rpo) : RepositoryService<Sys_Config, long, IConfigService>(rpo), IConfigService
{
    /// <inheritdoc />
    ...
    public async Task<IOrderedEnumerable<KeyValuePair<IImmutableDictionary<string, string>, int>>> CountByAsync(QueryReq<QueryConfigReq> req) {
        ...
        return ret
            .Select(x => new KeyValuePair<IImmutableDictionary<string, string>, int>(
                    req.RequiredFields.ToImmutableDictionary(y => y, y => typeof(Sys_Config).GetProperty(y)!.GetValue(x.Key)?.ToString()), x.Value
                )
            )
    ...
    public async Task<QueryConfigRsp> GetRegisterConfigAsync() {
        ...
        {
            RegisterInviteRequired = latestConfigAsync.RegisterInviteRequired, RegisterMobileRequired = latestConfigAsync.RegisterMobileRequired
        };
    }
    ...
    private ISelect<Sys_Config> QueryInternal(QueryReq<QueryConfigReq> req) {
        var ret = Rpo
            .Select.Include(a => a.UserRegisterDept)
            .Include(a => a.UserRegisterRole)
            .WhereDynamicFilter(req.DynamicFilter)
            .WhereIf(req.Filter?.Enabled.HasValue ?? false, a => a.Enabled == req.Filter.Enabled.Value);
