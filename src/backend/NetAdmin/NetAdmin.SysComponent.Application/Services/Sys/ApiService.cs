...
public sealed class ApiService(
    ...
    , IActionDescriptorCollectionProvider actionDescriptorCollectionProvider) : RedisService<Sys_Api, string, IApiService>(rpo), IApiService
{
    /// <inheritdoc />
    ...
    public async Task<IOrderedEnumerable<KeyValuePair<IImmutableDictionary<string, string>, int>>> CountByAsync(QueryReq<QueryApiReq> req) {
        ...
        return ret
            .Select(x => new KeyValuePair<IImmutableDictionary<string, string>, int>(
                    req.RequiredFields.ToImmutableDictionary(y => y, y => typeof(Sys_Api).GetProperty(y)!.GetValue(x.Key)?.ToString()), x.Value
                )
            )
    ...
    public IEnumerable<QueryApiRsp> ReflectionList(bool excludeAnonymous = true) {
        ...
        var actionDescriptors = actionDescriptorCollectionProvider.ActionDescriptors.Items.Cast<ControllerActionDescriptor>();

        if (excludeAnonymous) {
            actionDescriptors = actionDescriptors.Where(x => x.EndpointMetadata.All(y => y is AllowAnonymousAttribute));
        }

        var actionGroup = actionDescriptors.GroupBy(x => x.ControllerTypeInfo);
        ...
        QueryApiRsp SelectQueryApiRsp(IGrouping<TypeInfo, ControllerActionDescriptor> group) {
            ...
            var id = Regex.Replace(first.AttributeRouteInfo!.Template!, $"/{first.ActionName}$", string.Empty);
    ...
    private IEnumerable<QueryApiRsp> GetChildren(IEnumerable<ControllerActionDescriptor> actionDescriptors) {
        return actionDescriptors.Select(x =>
            {
