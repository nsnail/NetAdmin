    ...
    : RepositoryService<Sys_UserProfile, long, IUserProfileService>(rpo), IUserProfileService
{
    ...
    public static string BuildAppConfig(Dictionary<long, string> roles) {
        try {
            return new string[][]
            { [Chars.FLG_FRONT_APP_SET_HOME_GRID, new { content = roles.MaxBy(x => x.Key).Value.ToObject<object>(), datetime = 0 }.ToJson()]
            }.ToJson();
        }
        catch {
            return "[]";
    ...
    public async Task<IOrderedEnumerable<KeyValuePair<IImmutableDictionary<string, string>, int>>> CountByAsync(QueryReq<QueryUserProfileReq> req) {
        ...
        return ret
            .Select(x => new KeyValuePair<IImmutableDictionary<string, string>, int>(
                    req.RequiredFields.ToImmutableDictionary(y => y, y => typeof(Sys_UserProfile).GetProperty(y)!.GetValue(x.Key)?.ToString())
                    , x.Value
                )
            )
    ...
    public async Task<PagedQueryRsp<QueryUserProfileRsp>> PagedQueryAsync(PagedQueryReq<QueryUserProfileReq> req) {
        ...
        return new PagedQueryRsp<QueryUserProfileRsp>(
        ...
        return ret.ConvertAll(x => x.a.Adapt<QueryUserProfileRsp>() with
            {
                ...
                , EmergencyContactArea = x.e.Key == null ? null : x.e.Adapt<QueryDicContentRsp>()
            }
        );
    }
    ...
    private ISelect<Sys_UserProfile, Sys_DicContent, Sys_DicContent, Sys_DicContent, Sys_DicContent> QueryInternalComplex(
        ...
        var ret = Rpo
            .Orm.Select<Sys_UserProfile, Sys_DicContent, Sys_DicContent, Sys_DicContent, Sys_DicContent>()
            .LeftJoin((
                    a
                    , b
                    , _
            ...
            .LeftJoin((
                    ...
                    , __
                    , ___
                ) => a.CompanyArea.ToString() == c.Value && c.CatalogId == Numbers.ID_DIC_CATALOG_GEO_AREA
            )
            .LeftJoin((
                    a
                    , _
            ...
            .LeftJoin((
                    ...
                    , ___
                    , e
                ) => a.EmergencyContactArea.ToString() == e.Value && e.CatalogId == Numbers.ID_DIC_CATALOG_GEO_AREA
            )
