...
public sealed class CaptchaService : ServiceBase<ICaptchaService>, ICaptchaService
{
    ...
    public async Task<GetCaptchaRsp> GetCaptchaImageAsync() {
        var (backgroundImage, sliderImage, offsetSaw) = await CaptchaImageHelper
            .CreateSawSliderImageAsync(
                _currAsm, $"{_currAsmName}.Assets.Captcha.background", $"{_currAsmName}.Assets.Captcha.template", (1, 101), (1, 7), new Size(50, 50)
            )
