...
public sealed class DicService(IDicCatalogService catalogService, IDicContentService contentService) : ServiceBase<IDicService>, IDicService
{
    /// <inheritdoc />
    ...
    public async Task<string> GetDicValueAsync(GetDicValueReq req) {
        ...
        var df = new DynamicFilterInfo
        {
            ...
            [
                ...
                {
                    Field = nameof(QueryDicContentReq.CatalogId), Operator = DynamicFilterOperators.Eq, Value = req.CatalogId
                }
                , new DynamicFilterInfo { Field = nameof(QueryDicContentReq.Key), Operator = DynamicFilterOperators.Eq, Value = req.Key }
            ]
        };
