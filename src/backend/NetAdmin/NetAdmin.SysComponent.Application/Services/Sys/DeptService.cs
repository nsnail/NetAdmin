...
public sealed class DeptService(BasicRepository<Sys_Dept, long> rpo) : RepositoryService<Sys_Dept, long, IDeptService>(rpo), IDeptService
{
    /// <inheritdoc />
    ...
    public async Task<IOrderedEnumerable<KeyValuePair<IImmutableDictionary<string, string>, int>>> CountByAsync(QueryReq<QueryDeptReq> req) {
        ...
        return ret
            .Select(x => new KeyValuePair<IImmutableDictionary<string, string>, int>(
                    req.RequiredFields.ToImmutableDictionary(y => y, y => typeof(Sys_Dept).GetProperty(y)!.GetValue(x.Key)?.ToString()), x.Value
                )
            )
    ...
    public async Task<List<long[]>> GetChildDeptIdsAsync(long deptId) {
        var ret = await Rpo.Where(a => a.Id == deptId).AsTreeCte().ToListAsync(a => new { a.Id, a.ParentId }).ConfigureAwait(false);
        return ret.ConvertAll(x => new[] { x.Id, x.ParentId });
    }
    ...
    private ISelect<Sys_Dept> QueryInternal(
        QueryReq<QueryDeptReq> req
        , bool asTreeCte
    ) {
        var ret = Rpo
            ...
            .WhereIf(
                req.Keywords?.Length > 0, a => a.Id == req.Keywords.Int64Try(0) || a.Name.Contains(req.Keywords) || a.Summary.Contains(req.Keywords)
            );
        if (asTreeCte) {
            ret = ret.AsTreeCte();
        }
