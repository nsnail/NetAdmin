    ...
    : DistributedCache<IUserService>(cache, service), IScoped, IUserCache
{
    ...
    public Task<LoginRsp> LoginByUserIdAsync(LoginByUserIdReq req) {
        ...
        return GetOrCreateAsync(
            GetCacheKey(req.UserId.ToInvString()), () => Service.LoginByUserIdAsync(req), TimeSpan.FromSeconds(Numbers.SECS_CACHE_LOGIN_BY_USER_ID)
        );
        #else
        return Service.LoginByUserIdAsync(req);
        #endif
    }
    ...
    public Task<IEnumerable<QueryUserRsp>> QueryAsync(QueryReq<QueryUserReq> req) {
        #if !DEBUG
        return GetOrCreateAsync(
            GetCacheKey(req.Json().Crc32().ToString(CultureInfo.InvariantCulture)), () => Service.QueryAsync(req)
    ...
    public Task RemoveUserInfoAsync() {
        return RemoveAsync(GetCacheKey(Service.UserToken.Id.ToString(CultureInfo.InvariantCulture), nameof(UserInfoAsync)));
    }
