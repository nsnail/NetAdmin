...
public sealed class DicCache(IDistributedCache cache, IDicService service) : DistributedCache<IDicService>(cache, service), IScoped, IDicCache
{
    /// <inheritdoc />
    ...
    public Task<string> GetDicValueAsync(GetDicValueReq req) {
        #if !DEBUG
        return GetOrCreateAsync(                                                               GetCacheKey(req.Json().Crc32().ToString(CultureInfo.InvariantCulture))           , () => Service.GetDicValueAsync(req), TimeSpan.FromSeconds(Numbers.SECS_CACHE_DEFAULT));
        #else
