...
public sealed class UserAgentParser
{
    ...
    private bool SetMobile() {
        var kv = _mobiles.FirstOrDefault(x => _agent.Contains(x.Key, StringComparison.OrdinalIgnoreCase));
    ...
    private bool SetPlatform() {
        var kv = _platforms.FirstOrDefault(x => Regex.IsMatch(_agent, $"{Regex.Escape(x.Key)}", RegexOptions.IgnoreCase));
    ...
    private bool SetRobot() {
        var kv = _robots.FirstOrDefault(x => Regex.IsMatch(_agent, $"{Regex.Escape(x.Key)}", RegexOptions.IgnoreCase));
        if (kv.Key == null) {
            return false;
        }
