...
public sealed class ScheduledJob : WorkBase<ScheduledJob>, IJob
{
    ...
    public Task ExecuteAsync(CancellationToken cancelToken) {
        return Parallel.ForAsync(
            0, Numbers.SCHEDULED_JOB_PARALLEL_NUM, cancelToken, async (
                _
                , _
    ...
    protected override async ValueTask WorkflowAsync(CancellationToken cancelToken) {
        ...
        if (rsp.StatusCode is HttpStatusCode.Unauthorized or HttpStatusCode.Forbidden) {
            var loginRsp = await userService.LoginByUserIdAsync(new LoginByUserIdReq { UserId = job.UserId }).ConfigureAwait(false);
            ...
            .AtomicOperateAsync(async () =>
                {
                    ...
                    await jobService
                        .FinishJobAsync(job.Adapt<FinishJobReq>() with { LastStatusCode = (int?)rsp.StatusCode, LastDuration = jobRecord.Duration })
    ...
    private HttpRequestPart BuildRequest(QueryJobRsp job) {
        ...
        if (!_accessToken.NullOrEmpty()) {
            headers.Add(Chars.FLG_HTTP_HEADER_KEY_AUTHORIZATION, $"{Chars.FLG_HTTP_HEADER_VALUE_AUTH_SCHEMA} {_accessToken}");
        }

        if (!_refreshToken.NullOrEmpty()) {
            headers.Add(Chars.FLG_HTTP_HEADER_KEY_X_ACCESS_TOKEN_HEADER_KEY, $"{Chars.FLG_HTTP_HEADER_VALUE_AUTH_SCHEMA} {_refreshToken}");
        }
