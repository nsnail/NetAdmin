...
{
    #if DEBUG
    private static readonly Dictionary<Regex, string> _consoleColors = new()
    {
        { new Regex(@"(\d{2,}\.\d+ ?ms)", RegexOptions.Compiled), $"[{nameof(ConsoleColor.Magenta)}]$1[/]" }
        , { new Regex("(Tb[a-zA-Z0-9]+)", RegexOptions.Compiled), $"[{nameof(ConsoleColor.Cyan)}]$1[/]" }
        , { new Regex("(INSERT) ", RegexOptions.Compiled), $"[{nameof(ConsoleColor.Blue)}]$1[/] " }
        , { new Regex("(SELECT) ", RegexOptions.Compiled), $"[{nameof(ConsoleColor.Green)}]$1[/] " }
        , { new Regex("(UPDATE) ", RegexOptions.Compiled), $"[{nameof(ConsoleColor.Yellow)}]$1[/] " }
        , { new Regex("(DELETE) ", RegexOptions.Compiled), $"[{nameof(ConsoleColor.Red)}]$1[/] " }
        , { new Regex("(<s:.+?>)", RegexOptions.Compiled), $"[underline {nameof(ConsoleColor.Gray)}]$1[/] " }
        , { new Regex("(ResponseBody)", RegexOptions.Compiled), $"[underline {nameof(ConsoleColor.Cyan)}]$1[/] " }
        , { new Regex("(RequestBody)", RegexOptions.Compiled), $"[underline {nameof(ConsoleColor.Magenta)}]$1[/] " }
        , { new Regex(@"(\[\[dbo\]\]\.)(\[\[.+?\]\]) ", RegexOptions.Compiled), $"$1[{nameof(ConsoleColor.Magenta)}]$2[/] " }
    };
    ...
    public static IServiceCollection AddAllOptions(this IServiceCollection me) {
        var optionsTypes =
            from type in App.EffectiveTypes.Where(x =>
                !x.IsAbstract && !x.FullName!.Contains(nameof(Gurion)) && x.GetInterfaces().Contains(typeof(IConfigurableOptions))
            )
            select type;
        ...
        return me.AddConsoleFormatter(options =>
            {
                ...
                ) =>
                {
                    ...
                    Console.WriteLine(logFormat, date, logLevels[(LogLevels)message.LogLevel].ShortName, logName, message.ThreadId, msg);
                    #endif
    ...
    private static void MarkupLine(
        ...
        msg = _consoleColors.Aggregate(
            msg, (
                current
                , regex
            ) => regex.Key.Replace(current, regex.Value)
        );
        ...
        AnsiConsole.MarkupLine(CultureInfo.InvariantCulture, logFormat, date, colorName, logName, message.ThreadId, msg);
    }

    #endif
    private static (string Date, string LogName, string LogFormat) ParseMessage(
        LogMessage message
        , bool showColor
    ) {
