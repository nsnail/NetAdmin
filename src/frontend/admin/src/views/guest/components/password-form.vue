<template>
    <el-form :model="form" :rules="rules" @keyup.enter="login" label-width="0" ref="loginForm" size="large">
        <el-form-item prop="account">
            <el-input v-model="form.account" :placeholder="$t('用户名 / 手机 / 邮箱')" clearable prefix-icon="el-icon-user" />
        </el-form-item>
        <el-form-item prop="password">
            <el-input v-model="form.password" :placeholder="$t('请输入密码')" clearable prefix-icon="el-icon-lock" show-password />
        </el-form-item>
        <el-form-item style="margin-bottom: 1rem">
            <el-col :span="12">
                <el-checkbox v-model="autoLogin" :label="$t('24小时免登录')" />
            </el-col>
