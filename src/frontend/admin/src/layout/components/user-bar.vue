<template>
    <div class="user-bar">
        ...
        <div @click="search" class="panel-item hidden-sm-and-down user-bar-btn-search">
            <el-icon>
                <el-icon-search />
            </el-icon>
        </div>
...
export default {
    ...
    methods: {
        async getFailJobsCnt() {
            if (this.$GLOBAL.permissions.some((x) => x === '*/*/*' || x === 'system/job/user-bar')) {
