<template>
    <el-container>
        ...
        <el-main class="nopadding">
            <scTable ref="table" :apiObj="apiObj" row-key="id" @selection-change="selectionChange" stripe remoteSort
                     remoteFilter>
                ...
                <el-table-column label="无限权限" prop="enabled" width="80">
                    <template #default="scope">
                        <el-switch v-model="scope.row.ignorePermissionControl" disabled></el-switch>
                    </template>
                </el-table-column>
                <el-table-column label="创建时间" prop="createdTime" width="180" sortable='custom'></el-table-column>
    ...
    methods: {
        ...
        async batch_del() {
            this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？如果删除项中含有子集将会被一并删除`, '提示', {
                ...
                try {
                    const res = await this.$API.sys_role.bulkDelete.post({ids: this.selection.map(x => x.id)});
                    this.$refs.table.refresh()
