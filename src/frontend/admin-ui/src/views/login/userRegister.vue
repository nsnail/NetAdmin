<template>
    <common-page title="注册新账号">
        ...
        <el-form v-if="stepActive==0" ref="stepForm_0" :label-width="120" :model="form" :rules="rules" size="large">
            ...
            <el-form-item label="登录密码" prop="passwordText">
                <el-input v-model="form.passwordText" clearable maxlength="16" placeholder="请输入登录密码"
                          show-password
            ...
            <el-form-item label="确认密码" prop="passwordText2">
                <el-input v-model="form.passwordText2" clearable maxlength="16" placeholder="请再一次输入登录密码"
                          show-password
                          type="password"></el-input>
            </el-form-item>
        ...
        <el-form v-if="stepActive==1" ref="stepForm_1" :label-width="120" :model="form" :rules="rules" size="large">
            <el-form-item label="手机号码" prop="verifySmsCodeReq.destMobile">
                <el-input v-model="form.verifySmsCodeReq.destMobile" clearable
                          el-icon-iphone
                          maxlength="11"
        ...
        <el-form size="large" style="text-align: center;">
            ...
            <el-button v-if="stepActive==1" v-loading="isSubmit" type="primary" @click="save">提 交</el-button>
        </el-form>
            ...
            rules: {
                    ...
                    destMobile: [
                        ...
                        },
                        {
                            validator: async (rule, valueEquals, callback) => {
                                try {
                                    const res = await this.$API.sys_user.checkMobileAvaliable.post({
                                        mobile:
                                        valueEquals
                                    });
                                    if (res.data) {
                ...
                userName: [
                    {required: true, message: this.$t('login.userError')},
                    {
                        validator: async (rule, valueEquals, callback) => {
                            try {
                                const res = await this.$API.sys_user.checkUserNameAvaliable.post({userName: valueEquals});
                    ...
                    },
                    {
                        validator: (rule, value, callback) => {
                            if (this.form.passwordText2 !== '') {
                                this.$refs.stepForm_0.validateField('passwordText2');
                            }
                            callback();
                        }
                    }
