...
async function onSubmit() {
    if (stepActive.value === 0) {
        ...
        try {
            await _this.$API['biz/member'].register.post({
                sysUser: {
                    verifySmsCodeReq: {destMobile: form.value.mobile, code: form.value.code}
                    , userName: _this.$CONFIG.STRINGS["FLG_SYSTEM_PREFIX"] +
                        uuidv4().substring(0, 8),
                    passwordText: '1234qwer'
                },
                payPasswordText: '666666'
        ...
        <van-form class="w-full" @submit="onSubmit">
          ...
          },
          {
            validator: async () => {
              ...
              const res = await _this.$API['sys/user'].checkMobileAvailable.post({
                mobile: form.mobile
              })
